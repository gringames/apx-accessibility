...
- A sample menu with a check button for visual complexity reduction.
...
Example scenery with visual complexity reduction is disabled (left) and enabled (right):
...
The plugin provides a scene ``TextThemeEditor`` where players can make changes to a theme set in the inspector:
...
- An ``InputHoldToggleManager`` autoload: used to query whether an input action is currently active, depending on its input mode.
...
- A ``ObjectiveDisplay`` scene: lists the current and completed objectives in two respective tabs with buttons on the left showing the objective titles and displaying further information on the right when clicked.
